...
(function ($) {
  ...
  (function () {
    var $nav = $("#nav"),
      ...
      $controls = $(
        '<nav><span class="previous"></span><span class="next"></span></nav>'
      ).appendTo($main),
    ...
    $slides.each(function () {
      ...
      $this
        .css(
          "background-image",
          (bg.image ? bg.image + "," : "") + 'url("' + $img.attr("src") + '")'
        )
        .css("background-size", (bg.size ? bg.size + "," : "") + "cover")
        .css(
          "background-position",
          (bg.position ? bg.position + "," : "") + "0% 50%"
        )
        .css(
          "background-repeat",
          (bg.repeat ? bg.repeat + "," : "") + "no-repeat"
        )
        .css(
          "background-attachment",
          (bg.attachment ? bg.attachment + "," : "") + "fixed"
        );
      ...
      if (settings.parallax)
        $main.on("scroll", function () {
          if (
            ...
            $window.prop("orientation") == 0 ||
            $window.prop("orientation") == 180 ||
            $window.width() < $window.height()
          ) {
            if (position)
              $this.css(
                "background-position",
                (bg.position ? bg.position + "," : "") + position
              );
            else
              $this.css(
                "background-position",
            ...
            $this.css(
              "background-position",
              (bg.position ? bg.position + "," : "") + p * 100 + "% 50%"
            );
          }
        });
    });
